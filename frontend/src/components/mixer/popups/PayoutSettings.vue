    ...
    <v-card>
      <v-card-text class="pt-2">
        ...
        <v-radio-group v-model="payoutMethod">
          <v-radio label="Set relative payout percentages (recommended)" value="percentage" />
          <v-radio label="Set absolute payout amounts" value="amount" />
        </v-radio-group>
      </v-card-text>
      <v-card-actions>
        <v-btn color="primary" class="ml-auto" text @click="toggleSettingsPopup(false)">
          Close
        </v-btn>
      </v-card-actions>
    </v-card>
